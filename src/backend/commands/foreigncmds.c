						...
						Oid fdwvalidator)
{
	...
	/*
	 * Check and separate out the mpp_execute & segment_number option, fdwvalidator doesn't have
	 * to handle it. USER MAPPING doesn't have the options.
	 */
	if (catalogId != UserMappingRelationId)
	{
		SeparateOutMppExecute(&resultOptions);
		SeparateOutSegmentNumber(&resultOptions);
	}
